...
This document summarizes the **ping test** implementation and current results. **For a new chat:** read `agent/intro.md` → WIP doc → `context/current-state.md` (and `../README.md` for overview if needed) → this file and `datasheet-and-an-notes.md`; WIP doc has a **"Context for next session"** section with what's done and what to try next.
...
- **Pins:** Same as GPIO/SPI tests — `sx1262_gpio_test/pin_config.py` and `context/wiring.md`.
...
**Datasheet/AN:** See **`datasheet-and-an-notes.md`**. ClearDeviceErrors 0x00,0x00. CalibrateImage band bytes per datasheet/RadioLib (863–870: 0xD7,0xDB; 902–928: 0xE1,0xE9). decode_device_error() logs set bits on TX failure.
...
**Key excerpts from the Semtech SX1262 datasheet, AN1200.59, and Seeed Wio-SX1262-N datasheet are in `datasheet-and-an-notes.md` in this folder** (TCXO command and delay, error register, Tx sequence). The PDFs do not need to remain in the repo.
