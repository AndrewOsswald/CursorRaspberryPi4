...
- **Not working / limitations:** Ping test **fails (no TxDone)**; **error 0x200A** → RC13M_CALIB_ERR, ADC_CALIB_ERR (block calibration fails on chip). Driver is RadioLib-aligned: retry Standby, TCXO then config, CalibrateImage **2-byte** band params (868: 0xD7,0xDB), regulator before XOSC, SetPaConfig. See `PING_TEST_SUMMARY.md` **“What we learned (for next session)”** and task doc **“Context for next session”** for full handoff. Next: hardware (antennas, one-module test), or run known-good stack on same Pi and diff.
